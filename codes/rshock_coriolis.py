...
def rshock(liste_param):
	...
	from raytracing import RT
        ...
        Mdot1,Mdot2,vinf1,vinf2, mass_ratio,ex, omega, a, per,R1,R2,beta1,beta2,Teff_1, Teff_2, nbr_points_shock_2D, nbr_points_shock_3D, nstep_width, nbr_bin_profile, direct, cut_lim, wind_prim, wind_sec, phase, incl,ipar, mu, T, M_conj=liste_param
	...
	for ibal in range(nbr_bal+1):
		...
		for icut in range(nbr_points_shock_3D):
			...
			if (ibal > 0):
				...
				vol_cd=np.concatenate((vol_cd,l0_1*(xcut**2+ycut**2+zcut**2)*dtheta*dphi/(abs(math.sin(slope_vech-thetah))*2.*nstep_width)+0.*np.arange(nstep_width)))
